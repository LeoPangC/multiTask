...
if __name__ == '__main__':
    ...
    wind_train = GetWindSet(gfs_data_1=gfsu_data[:100], era5_data_1=era5u_data[:100], gfs_data_2=gfsv_data[:100],
                            era5_data_2=era5v_data[:100])
    train_dataloader = DataLoader(wind_train, batch_size=2, shuffle=False, drop_last=False, num_workers=2)

    net = UNet()
    for epoch in range(200):  # epoch:50
        # generator.train()
        # discriminator.train()
        # D_train_epoch_loss = []
        G_train_epoch_loss = []
        for i, (dataX, dataY) in enumerate(train_dataloader):
            x = torch.cat((dataX, dataX), dim=1)
            output = net(x, 20)
            pass